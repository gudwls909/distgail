...
####################  distgail discrim network
class Discrim_Network(BaseNetwork):
    def __init__(self, D_in1, D_out, head="mlp", D_hidden=512):
        D_head_out = super(Discrim_Network, self).__init__(D_in1, D_hidden, head)
        self.l = torch.nn.Linear(D_head_out, D_hidden)
        self.q = torch.nn.Linear(D_hidden, D_out)
    ...
    def forward(self, x):
        x = super(Discrim_Network, self).forward(x)
        x = F.relu(self.l(x))
        x = F.sigmoid(self.q(x))  # input state, output D(s,a) 0~1

        #x = torch.log(x)  # output log(D(s,a))
        return x

class Continuous_Discrim_Network(BaseNetwork):
    def __init__(self, D_in1, D_in2, head="mlp", D_hidden=512):
        D_head_out = super(Continuous_Discrim_Network, self).__init__(D_in1, D_hidden, head)
        self.e = torch.nn.Linear(D_in2, D_hidden)
        self.l = torch.nn.Linear(D_hidden + D_head_out, D_hidden)
        self.q = torch.nn.Linear(D_hidden, 1)
    ...
    def forward(self, x1, x2):
        x1 = super(Continuous_Discrim_Network, self).forward(x1)
        x2 = F.relu(self.e(x2))
        x = torch.cat([x1, x2], dim=-1)
        x = F.relu(self.l(x))
        x = F.sigmoid(self.q(x))  # input state and action, output D(s,a) 0~1
        return x
########################
